...
In this next step you install Flux using `helm`. Simply
 ...
 1. Install Flux and the Helm operator by specifying your fork URL:
    ...
    - Using a public git server from `bitbucket.com`, `github.com`, `gitlab.com`, `dev.azure.com`, or `vs-ssh.visualstudio.com`:
      ...
      --set git.url=git@github.com:prvnmali2017/flux-get-started \
      --namespace flux \
      fluxcd/flux
      ```
...
deployed into the `flux` namespace) listed like so:
...
change the `env variable` argument.
...
Confirm the change landed by checking the `flux` namespace that
Flux is deploying to:

```sh
kubectl describe -n flux deployment/<dep_name>
```
